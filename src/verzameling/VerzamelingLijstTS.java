...
public class VerzamelingLijstTS implements IFotoVerzameling {
    private TreeSet<Foto> lijst;
    private Comparator comp;
    
    /**
     * Default constructor
     */
    public VerzamelingLijstTS() {
        comp = new NaamComparator<Foto>();
        lijst = new TreeSet(comp);        
    }
    ...
    public Foto get(int index) throws VerzamelingException {
        Foto foto;
        List<Foto> aslist = toList();
        try {
            foto = aslist.get(index);
            return foto;
        }
    ...
    public void sort(Comparator comp) {
        if (comp != this.comp){ // we use a different comapare method and we have to rebuild
            this.comp = comp;
            TreeSet<Foto> tmp = new TreeSet(comp);
            tmp.addAll(lijst);
            lijst = tmp;
        }
    }
